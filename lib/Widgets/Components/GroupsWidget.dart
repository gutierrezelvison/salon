...
class _GroupsWidgetState extends State<GroupsWidget> {
  ...
  void _saveForm() async {
    ...
    if (isValid) {
        if (edit) {
          if (await bdConnection.updateGroup(context: context,id: idSucursal,name: _localNameController.text.toLowerCase(), status: status?'active':'deactive',)) {
            limpiar();
            CustomSnackBar().show(
                context: context,
          ...
          if (await bdConnection.setGroup(context: context,name: _localNameController.text.toLowerCase(),
              status: status?'active':'deactive')) {
            limpiar();
            CustomSnackBar().show(
                context: context,
                msg: 'Grupo agregada con éxito!',
                icon: Icons.check_circle_outline_rounded,
                color: const Color(0xff22d88d)
            );
          }else{
  ...
  deleteItem(int id)async{
    if(await bdConnection.deleteGroup(context: context,id: id)){
      setState(() {
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(pageIndex==0?"Grupos de empleados":edit?"Modificar Grupo":"Añadir Grupo",
  ...
  Widget sucursales(){
    return FutureBuilder(
        future: bdConnection.getGroups(context),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    SizedBox(width: responsiveApp.setWidth(10),),
                    texto(text: 'Estado', size: responsiveApp.setSP(12)),
                  ],
                ),
                const SizedBox(
                  height: 20,
